...
add_bing_images <- function(questions) {
  questions %>%
    mutate(
      image_A = if_else(is.na(image_A), get_first_bing_image(answer_A), as.character(image_A)),
      image_B = if_else(is.na(image_B), get_first_bing_image(answer_B), as.character(image_B)),
      image_C = if_else(is.na(image_C), get_first_bing_image(answer_C), as.character(image_C)),
      image_D = if_else(is.na(image_D), get_first_bing_image(answer_D), as.character(image_D))
    )
}
